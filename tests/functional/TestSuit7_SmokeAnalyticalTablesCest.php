...
class SmokeAnalyticalTablesCest {
    ...
    public function AnalyticalTablesModule(AcceptanceTester $I) {

        $locator = '//ul[@class="nav-tabs nav-tabs-limited-height"]';

        $this->perehod->open_object_and_module($I, 128, 'fin_tables');

        foreach ($this->locator->locator_analytical_tables as $locat_module => $element_visibly) {
